% IEEE Access Resubmission (Access-2026-02049)
% Clean preamble: two-column figures, table polish, compact listings,
% algorithms, FloatBarrier, and IEEE Access compatibility.
% =========================================================================
...
% ---------------------------------------------------------------------------
% Compact listing style for IEEE two-column pages.
% ---------------------------------------------------------------------------
...
\author{\uppercase{Golam Bari Fardeen, Namare Shakib Angkon, Syed Zayan
Anwar, Md.\ Masum Mizi, Shinthia Rahman, and Md.\ Motaharul Islam
(Member, IEEE)}}
...
\markboth{Fardeen \headeretal: Secure Legal Data Management With Fabric and IPFS}{Fardeen \headeretal: Secure Legal Data Management With Fabric and IPFS}

\corresp{Corresponding author: Md.\ Motaharul Islam
(email: motaharul@cse.uiu.ac.bd).}

\tfootnote{This work was supported by the Institute for Advanced
Research Publication Grant of United International University, under
Grant IAR 2025 Pub 093.}
...
\begin{abstract}
Legal document management systems often centralize trust in privileged administrators, making it difficult for external parties to verify whether access decisions or audit records have been altered. This paper presents a hybrid legal-data management framework that places Hyperledger Fabric on the active document-release path, rather than using it only as a passive audit log. In the proposed architecture, every protected download request must pass a Fabric chaincode access check before ciphertext or wrapped document keys are released. If Fabric is unavailable, the evaluated download path fails closed and returns a denial instead of falling back to a database access-control list. The framework combines browser-side encryption, encrypted off-chain storage in a private IPFS swarm, and on-chain anchoring of document hashes and content identifiers. Experimental evaluation shows that the REST-gateway workload sustains up to 193 transactions per second under the tuned configuration, exceeding the estimated workload baseline for a 1,000-user firm. Fabric-based add no statistically significant interactive delay compared with a database-only access check, and outage simulations confirm that successful downloads drop to zero during Fabric unavailability while service resumes automatically after reconnection. The main contribution is an empirical systems baseline for moving legal-document release decisions from application-only authorization to ledger-verifiable access-control state on the normal download path.
\end{abstract}
...
\begin{keywords}
Blockchain, Hyperledger Fabric, Legal Document Management, Chaincode, Data Integrity, Access Control, Role-Based Access Control,
Digital Forensics.
\end{keywords}
...
empirically confirmed for the download path.
...
\begin{table*}[!t]
\centering
\caption{Comparison with representative blockchain-based legal and evidence-management systems. 
...
framework. First, many blockchain-based evidence systems provide
tamper-evident storage or timestamping but leave authorization to the
application layer. This means a compromised or overly privileged
application server may still authorize access using state that is not
independently verifiable at the time of access. Second, access-control
models are often coarse-grained or static. Legal collaboration
...
signed the document content or action being recorded. Fourth,
off-chain storage availability is frequently assumed rather than
governed; IPFS content must be pinned and replicated deliberately.
...
\setlength{\tabcolsep}{4pt}
\footnotesize
\begin{tabularx}{\textwidth}{|>{\RaggedRight\arraybackslash}p{3.05cm}|>{\RaggedRight\arraybackslash}X|>{\RaggedRight\arraybackslash}X|>{\RaggedRight\arraybackslash}X|>{\RaggedRight\arraybackslash}X|}
\hline
\textbf{Property} &
...
Listing~\ref{lst:encrypt} shows the browser implementation.
...
Listing~\ref{lst:pbkdf2} shows the derivation function. Experiment~\hyperref[sec:exp_crypto]{6} (Section~\ref{sec:exp_crypto}) measures PBKDF2-SHA256 at
...
Listing~\ref{lst:ecdsa} shows the server-side verifier.
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=\columnwidth]{access_and_decryption.pdf}
...
layer. Listing~\ref{lst:download} shows the two-layer
...
Listing~\ref{lst:audittrigger} shows both triggers. These triggers
...
\scriptsize
\begin{tabularx}{\textwidth}{|p{2.2cm}|X|p{2.0cm}|X|X|}
\hline
...
\small
\begin{tabularx}{\textwidth}{|p{3cm}|X|p{1.4cm}|X|}
\hline
...
Figs.~\ref{fig:ui_overview} and~\ref{fig:ui_audit_views} show the
prototype screens used during a normal user session. The registration
screen (Fig.~\ref{fig:ui_registration}) collects a username, email
address, password, and role selection before account creation. The
dashboard (Fig.~\ref{fig:ui_dashboard}) is the landing page, presenting
the primary navigation cards for case work, documents, communication,
and audit-oriented tasks. The case-management view
(Fig.~\ref{fig:ui_case_management}) displays active matters as cards with
...
The document-details dialog (Fig.~\ref{fig:ui_document_details}) lists the
...
(Fig.~\ref{fig:ui_document_download}) shows the selected file, access
...
The transparency views separate record browsing from event review. The
ledger explorer (Fig.~\ref{fig:ui_ledger_explorer}) groups recent entries
by block and displays transaction IDs, channels, timestamps, transaction
types, and document references. The audit-log view
...
\subfloat[Registration]{%
    \includegraphics[width=0.465\linewidth]{ui_registration.png}%
    \label{fig:ui_registration}}\hfill
\subfloat[Dashboard]{%
    \includegraphics[width=0.465\linewidth]{ui_dashboard.png}%
    \label{fig:ui_dashboard}}\\[0.25em]
\subfloat[Case management]{%
    \includegraphics[width=0.465\linewidth]{ui_case_management.png}%
    \label{fig:ui_case_management}}\hfill
\subfloat[Document details (TxID + integrity hash)]{%
    \includegraphics[width=0.465\linewidth]{ui_document_details.png}%
    \label{fig:ui_document_details}}
\end{minipage}%
}
...
\subfloat[Document download (ECIES-wrapped key unwrapped client-side)]{%
    \includegraphics[width=\linewidth]{ui_document_download.png}%
    \label{fig:ui_document_download}}
\end{minipage}\hfill
\begin{minipage}[c]{0.58\linewidth}
\centering
\subfloat[Ledger explorer]{%
    \includegraphics[width=0.82\linewidth]{ui_ledger_explorer.png}%
    \label{fig:ui_ledger_explorer}}\\[0.25em]
\subfloat[Audit log]{%
    \includegraphics[width=0.82\linewidth]{ui_audit_log.png}%
    \label{fig:ui_audit_log}}
\end{minipage}
\end{minipage}%
}
\caption{Audit and transparency views.}
...
observed write-capacity reference ($\approx$279\,TPS), and CPU utilisation
...
by concurrency. This experiment characterises the throughput--latency
...
learning classifiers trained on \texttt{GetHistoryForKey} query
sequences to flag unusual access frequency or bulk downloads. The
append-only \texttt{audit\_log} table provides a low-latency feed
(3.9\,ms P50) suitable for near-real-time
analysis~\cite{siddamsetti2024anomaly}.

\subsubsection{Privacy-Preserving Access Verification}
...
checks~\cite{groth2016}, together with batching or cover-traffic
mechanisms to reduce timing and frequency leakage~\cite{fabricPDCDoc}.

\subsubsection{Formal Verification of Chaincode}
The \texttt{CheckAccess} and \texttt{RegisterDocument} chaincode
...
policy~\cite{permenev2020verx}.

\subsubsection{Comparative Complexity and Cost Modeling}
The present evaluation is empirical and prototype-focused. It compares the
measured Fabric \texttt{CheckAccess} path with a PostgreSQL-only ACL read path
...
Fabric-as-audit-log designs, and Fabric-on-path \texttt{CheckAccess}
...
\subsubsection{Decentralized Identity Integration}
Replacing the prototype's centralized PostgreSQL identity database
with W3C Decentralized Identifiers (DIDs)~\cite{sporny2023did}
...
\subsubsection{Cross-Network Evidence Collection}
...
exchange~\cite{sporny2022vc}.

\subsubsection{Cross-Version CID Chaining}
...
\subsubsection{IPFS Persistence Incentive Mechanism}
...
with FIPS~140-3-compliant key storage~\cite{fips1403,fabricCADoc},
and client-side WebAuthn/FIDO2 hardware authenticators replacing the
current \texttt{localStorage} key store. Both upgrades require no
chaincode modification~\cite{nist800634,w3cWebAuthnL3,nist800131a}.

\subsubsection{Key Recovery and Escrow Governance}
\label{sec:key_recovery_escrow}
The current prototype does not implement production-grade key recovery.
Because client private keys are stored in browser \texttt{localStorage}
...
\subsubsection{Regulator-Facing Compliance Interfaces}
...
(e.g., \texttt{AND('LawFirmAMSP.peer', 'RegulatorMSP.peer')}) requiring
joint regulator-firm approval for high-value asset transfers. These
extensions require no chaincode redesign, since \texttt{CheckAccess} and
\texttt{GetHistoryForKey} already support additional endorsement and
...
affirmative within the evaluated scope. Access-control \emph{state} ---
...
Beyond the quantitative results, this work makes four contributions. First,
it demonstrates a practical shift in the enforcement boundary from
application-only trust to ledger-verifiable access-control state on the normal
document-release path. Second, the prototype implements time-bounded grants
and revocation logic in chaincode, including expiry evaluation during access
checks. Third, it introduces a dual-store audit design that separates
low-latency operational queries from ledger-verifiable history. Fourth, it
provides an evidence-backed performance evaluation covering throughput,
latency, file-size effects, audit queries, WAN delay, cryptographic overhead,
ledger-history depth, \texttt{BatchTimeout} sensitivity, and fail-closed
outage behavior.
...
download path. The negligible read overhead measured here (6.51\,ms
vs.\ 7.16\,ms P50, a 0.65\,ms difference, $n{=}100$) confirms that this
architectural choice imposes negligible interactive overhead. It also establishes a reproducible baseline
...
% =========================================================================
\nopagebreak
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{fardeen.jpeg}}]{Golam Bari Fardeen}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{angkon.jpg}}]{Namare Shakib Angkon}
is currently pursuing his Bachelor of Science (B.Sc.) in Computer Science and Engineering from United International University (UIU), with a specialization in Data Science. His research interests are centered on machine learning, Natural Language Processing (NLP), deep learning, digital image processing, blockchain technology, and computer security.
\end{IEEEbiography}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{zayan.jpg}}]{Syed Zayan Anwar}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{Masum.png}}]{Md. Masum Mizi}
is currently pursuing his Bachelor of Science (B.Sc.) in Computer Science and Engineering from United International University (UIU), with a specialization in data science. His technical skills include a variety of programming languages, and his professional interests lie in software architecture and backend development.
\end{IEEEbiography}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{shinthia.jpg}}]{Shinthia Rahman}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{motaharul.jpg}}]{Md. Motaharul Islam (Member, IEEE)}
