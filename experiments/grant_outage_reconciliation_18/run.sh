...
# ── environment record ────────────────────────────────────────────────────
{
  ...
  echo "  \"channel\": \"$CHANNEL\", \"chaincode\": \"$CC committed: $(docker exec "$CLI" peer lifecycle chaincode querycommitted -C "$CHANNEL" -n "$CC" 2>/dev/null | grep -oE "Version: [^,]+, Sequence: [0-9]+" || echo unknown)\","
