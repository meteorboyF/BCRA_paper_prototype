#!/usr/bin/env node
/**
 * Two-host copy of the CANONICAL duration60s closed-loop harness
 * (experiments/caliper/pangochain-loadtest-configurable.js). The ONLY
 * functional change is that the gateway host/port/prefix come from
 * PANGOCHAIN_API_URL instead of being hardcoded to localhost:8080
 * (the original documented the variable but ignored it). Workload,
 * write ratio, timeout, closed-loop dispatch, and the output line are
 * byte-faithful to the original so results remain comparable.
 *
 * Env vars:
 *   PANGOCHAIN_API_URL         — gateway base URL incl. /api prefix
 *                                (e.g. http://192.168.0.10:8080/api)
...
const API_URL     = new URL(process.env.PANGOCHAIN_API_URL || 'http://localhost:8080/api');
const API_HOST    = API_URL.hostname;
const API_PORT    = parseInt(API_URL.port || '80', 10);
const API_PREFIX  = API_URL.pathname.replace(/\/$/, '');
...
function makeRequest(method, path, body) {
  return new Promise((resolve, reject) => {
    const options = {
      ...
      path: API_PREFIX + path, method, agent,
